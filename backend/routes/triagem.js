...
router.get('/', async (req, res) => {
  try {
    const db = getDB();
    const osComTriagem = await qa(db, `SELECT DISTINCT o.id, o.numero_os, o.cliente, o.equipamento, o.tipo, o.prioridade, o.data_abertura, (SELECT COUNT(*) FROM pecas_os p2 WHERE p2.os_id = o.id AND p2.status_entrega = 'Aguardando Triagem') as pendentes_triagem, (SELECT COUNT(*) FROM pecas_os p2 WHERE p2.os_id = o.id) as total_pecas FROM ordens_servico o JOIN pecas_os p ON p.os_id = o.id WHERE p.status_entrega = 'Aguardando Triagem' ORDER BY o.data_abertura DESC`);
    res.json(osComTriagem.map(os => ({ ...os, pendentes_triagem: parseInt(os.pendentes_triagem)||0, total_pecas: parseInt(os.total_pecas)||0 })));
