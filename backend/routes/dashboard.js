...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const [por_status, por_prioridade, val, atrasadas, transito, pendentes, mk, recentes, pecas_atrasadas, os_vencendo] = await Promise.all([
      ...
      q(`SELECT COUNT(*) as total FROM ordens_servico WHERE data_conclusao_estimada < CURRENT_DATE AND status != 'Concluída'`),
      ...
      qa(`SELECT p.id, p.descricao, p.data_entrega_prevista, p.status_entrega, o.numero_os, o.cliente, o.prioridade, o.id as os_id, CAST(EXTRACT(DAY FROM NOW() - p.data_entrega_prevista::timestamp) AS INTEGER) as dias_atraso FROM pecas_os p JOIN ordens_servico o ON p.os_id = o.id WHERE p.data_entrega_prevista < CURRENT_DATE AND p.status_entrega NOT IN ('Entregue','Cancelado') AND o.status != 'Concluída' ORDER BY dias_atraso DESC LIMIT 10`),
      qa(`SELECT id, numero_os, cliente, data_conclusao_estimada, prioridade, status, CAST(EXTRACT(DAY FROM data_conclusao_estimada::timestamp - NOW()) AS INTEGER) as dias_restantes FROM ordens_servico WHERE data_conclusao_estimada BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days' AND status != 'Concluída' ORDER BY data_conclusao_estimada ASC LIMIT 10`),
    ]);
