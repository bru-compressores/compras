...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const [por_status, por_prioridade, val, atrasadas, transito, pendentes, mk, recentes, pecas_atrasadas, os_vencendo, triagem, ag_aprovacao, bloqueadas] = await Promise.all([
      ...
      q(`SELECT COUNT(DISTINCT os_id) as total FROM pecas_os WHERE status_entrega = 'Aguardando Triagem'`),
      q(`SELECT COUNT(*) as total FROM ordens_servico WHERE status = 'Aguardando Aprovação Técnica'`),
      q(`SELECT COUNT(DISTINCT os_id) as total FROM pecas_os WHERE status_entrega = 'Bloqueado no Fornecedor'`),
    ]);
    ...
    res.json({
      ...
      os_aguardando_triagem: parseInt(triagem?.total) || 0,
      os_aguardando_aprovacao: parseInt(ag_aprovacao?.total) || 0,
      pecas_bloqueadas_fornecedor: parseInt(bloqueadas?.total) || 0
    });
