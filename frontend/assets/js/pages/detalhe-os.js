const PageDetalheOS = {
  ...
  badgeStatus(s) {
    const map = {
      ...
      'Aguardando Aprovação Técnica': { bg:'#f5f3ff', cor:'#7c3aed', dot:'#7c3aed' },
  ...
  renderTabelaPecas() {
    ...
    const rows = dadosOrdenados.map(p => {
      ...
      return '<tr onclick="PageDetalheOS.editarPecaInline(' + p.id + ')" style="cursor:pointer' + bgLinha + '">' +
        ...
        ['Aguardando Triagem','Separado (Almoxarifado)','Pendente','Em cotação','Bloqueado no Fornecedor','Aguardando Aprovação Técnica','Pedido realizado','Em trânsito','Entregue','Cancelado'].map(s =>
