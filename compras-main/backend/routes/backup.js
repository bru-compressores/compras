...
router.post('/restaurar', express.json({ limit: '50mb' }), async (req, res) => {
  ...
  res.json({ mensagem: 'Restauração iniciada. Aguarde alguns minutos e recarregue a página.', processando: true });
  ...
  (async () => {
    ...
    try {
      ...
      for (let i = 0; i < fornecedores.length; i += 20) {
        await Promise.all(fornecedores.slice(i, i+20).map(async f => {
          try { if (!await q(db,'SELECT id FROM fornecedores WHERE nome = ?',f.nome)) { await qr(db,'INSERT INTO fornecedores (nome,cnpj,contato,telefone,email,cidade,estado) VALUES (?,?,?,?,?,?,?)',f.nome,f.cnpj||null,f.contato||null,f.telefone||null,f.email||null,f.cidade||null,f.estado||null); forn_imp++; } } catch(e) {}
        }));
      }
      const mapaOS = {};
      const oss = (dados.ordens_servico||[]).filter(os => os.numero_os);
      for (let i = 0; i < oss.length; i += 10) {
        await Promise.all(oss.slice(i, i+10).map(async os => {
          try {
      ...
      for (let i = 0; i < pecas.length; i += 20) {
        await Promise.all(pecas.slice(i, i+20).map(async p => {
      ...
      console.log(`✅ Restauração: ${os_imp} OS, ${pecas_imp} peças, ${forn_imp} fornecedores`);
    } catch(e) { console.error('Erro restauração:', e.message); }
  })();
});
...
router.get('/status', async (req, res) => {
  try {
    ...
    res.json({ ordens_servico: parseInt(os.total)||0, pecas_os: parseInt(pecas.total)||0, fornecedores: parseInt(forn.total)||0 });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
